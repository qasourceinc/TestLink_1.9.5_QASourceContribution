...
function renderExecTreeNode($level,&$node,&$tcase_node,$hash_id_descr,
                            ...
                            $showTestCaseID,$testCasePrefix,$showTestSuiteContents,$plan_id=0)
{
	static $resultsCfg;
	static $l18n;	
	static $pf;	
	...
	{ 
		 ...
		 * Author : QASource
		 * Defect ID : 5751
		 * On hitting first level node in tree hierarchy ajaxRequest function is called
		 *				 
		 **/
		// $pf['testsuite'] = $hideTestCases ? 'TPLAN_PTS' : ($showTestSuiteContents ? 'STS' : null); 
		$pf['testsuite'] = $hideTestCases ? 'TPLAN_PTS' : ($showTestSuiteContents ? 'STS' : 'ajaxRequest'); 
		/* updated code ends */
	...
	switch($node_type)
	{
		...
		case 'testsuite':
			 ...
			 * Author : QASource
			 * Defect ID : 5751
			 * Show count of numbers according to execution
			 * We have implemented the else condition to show the record count of pass, fail, blocked test cases. 
			 *				 
			 **/
			if(($_GET['feature'] == 'tc_exec_assignment' || $_GET['feature'] == 'test_urgency' || $_GET['feature'] == 'planUpdateTC' || $_GET['type'] == 'testplan'|| $_GET['type'] == 'testreport' )){
				$testcase_count = isset($node['testcase_count']) ? $node['testcase_count'] : 0;	
				...
				$pass		=	!empty($node['pass']) ? $node['pass'] : 0;
				$fail		=	!empty($node['fail']) ? $node['fail'] : 0;
				$blocked	=	!empty($node['blocked']) ? $node['blocked'] : 0;
				$notRun		=	!empty($node['notRun']) ? $node['notRun'] : 0;
				$testcase_count = isset($node['total']) ? $node['total'] : 0;
				$node['text'] = $name ." (" . $testcase_count . ") (<font color='green'>".$pass."</font>,<font color='red'>".$fail."</font>,<font color='blue'>".$blocked."</font>,<font color='black'>".$notRun."</font>)";
				$node['text'] .=	"</span></a><div style='display:none' id={$node['id']}></div><a><span>";
			}
			/* updated code ends */
		break;
	 ...
	 * Author : QASource
	 * Defect ID : 5751
	 * On hitting 1st level node we set the function ajaxRequest for ajax call, We have implemented the if condition
	 *				 
	 **/
	$node['position'] = isset($node['node_order']) ? $node['node_order'] : 0;
	if($pf['testsuite'] == 'ajaxRequest'){
		$node['href'] = is_null($pfn)? '' : "javascript:{$pfn}({$node['id']},{$versionID},{$plan_id},{$node['parent_id']},{$node['build_id']})";
	}else{
	...
	if (isset($node['childNodes']) && $node['childNodes'])
	{
		...
		for($idx = 0;$idx <$nodes_qty ;$idx++)
		{
			...
			$menustring .= renderExecTreeNode($level+1,$node['childNodes'][$idx],$tcase_node,
			                                  ...
			                                  $testCasePrefix,$showTestSuiteContents,$plan_id);
		}
	}
	return $menustring;
}
