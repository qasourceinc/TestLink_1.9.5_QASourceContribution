...
class database 
{
	...
	function getRoleId($user_id,$limit = -1){
	...
	function getGrandParentName($grandParentID,$limit = -1){
		$sql	=	"SELECT prefix FROM testprojects WHERE id=".$grandParentID;
		$result = 	$this->exec_query($sql,$limit);
		if ($result)
		{
			$row 		= 	$this->fetch_array($result);
	...
	function getExternalID($tcversion_id,$limit = -1){
		$sql	=	"SELECT tc_external_id FROM tcversions WHERE id=".$tcversion_id;
		$result = 	$this->exec_query($sql,$limit);
		if ($result)
		{
			$row 		= 	$this->fetch_array($result);
			$tc_external_id	=	$row['tc_external_id'];
			return $tc_external_id;
		}
		return 0;
	}
	function createTempTable($sql,$limit = -1){
		$this->exec_query($sql,$limit);
	}
	function insertInTempTable($sql,$limit = -1){
		$this->exec_query($sql,$limit);
	}
	function dropTempTable($sql,$limit = -1){
		$this->exec_query($sql,$limit);
	}
