...
*/
	...
	*/
	 ...
	 *
	 */
	function getSkeleton($id,$tprojectID,$filters=null,$options=null,$build_id=0)
	{
		$debugMsg = 'Class:' . __CLASS__ . ' - Method: ' . __FUNCTION__;
		...
		$qnum = $this->$method2call($id,$tprojectID,$items,$my['filters'],$my['options'],$build_id);
		/* echo "<pre>";
		print_r($items); */
	 	return $items;
	}
	 ...
	 *
	 */
	function _get_subtree_rec($tplan_id,$node_id,&$pnode,$filters = null, $options = null,$build_id=0)
	{
		static $qnum;
		static $my;
		...
		if (!$my)
		{
			 ...
			 * Author : QASource
			 * Defect ID : 5751
			 * We have implemented the else condition for filters and temp table.
			 * The objective of temp table is to only count the test cases and their pass, fail, blocked count recursively as for filters input.
			 * The final query creates the php array object that contains test cases count generated from temp table.
			 * Here we have included the file "testplan.class.filter.manual_test.php". This file includes the logic build the temp table.
			 * It is on else part
			 * 
			 *				 
			 **/
			// $role_id	=	@mysql_result(mysql_query('select role_id from users where id='.$_SESSION['userID']),0);
			$role_id	=	$this->db->getRoleId($_SESSION['userID']);
			$full_access=	0;
			$case		=	0;
			if($role_id == 8){
				$full_access	=	1;
			}
			if(/* isset($_GET['feature']) &&  */($_GET['feature'] == 'tc_exec_assignment' || $_GET['feature'] == 'test_urgency' || $_GET['feature'] == 'planUpdateTC' || $_GET['type'] == 'testplan' || $_GET['type'] == 'testreport' )){
				...
				include("testplan.class.filter.manual_test.php");
		...
		if(/* isset($_GET['feature']) &&  */($_GET['feature'] == 'tc_exec_assignment' || $_GET['feature'] == 'test_urgency' || $_GET['feature'] == 'planUpdateTC' || $_GET['type'] == 'testplan' || $_GET['type'] == 'testreport' )){
			 ...
			 * Author QASource
			 * Defect ID : 5751
			 * Dropped temp table after getting result
			 *				 
			 **/
			$this->db->dropTempTable('DROP TEMPORARY TABLE IF EXISTS stored_result');
		 ...
		 * Author : QASource
		 * Defect ID : 5751
		 * We have implemented the else condition in order to load the tree without calling the function recursively.
		 * Changed the original fucntionality and recursive call of queries were divided into a single query for the first level nodes.  
		 *
		 **/
		$i		=	0;
		$t_cnt	=	0;
		...
		if(/* isset($_GET['feature']) &&  */($_GET['feature'] == 'tc_exec_assignment' || $_GET['feature'] == 'test_urgency' || $_GET['feature'] == 'planUpdateTC' || $_GET['type'] == 'testplan' || $_GET['type'] == 'testreport' )){
			...
			foreach($rs as $row)
			{
				...
				{  
					...
					$pnode['notRun']	=	$notRun;
					$pnode['build_id']	=	$build_id;
					$pnode['case']		=	$case;
					
					
					$t_cnt					=	$t_cnt + $row['count'];
