...
function execTree(&$dbHandler,&$menuUrl,$tproject_id,$tproject_name,$tplan_id,
                  $tplan_name,$objFilters,$objOptions,$build_id=0) 
{
    ...
    $test_spec = $tplan_mgr->getSkeleton($tplan_id,$tproject_id,$my['filters'],$my['options'],$build_id);
 	//Echo 'BEFORE';
	...
	if($test_spec)
	{
		...
		if( !is_null($targetExecStatus) && (!in_array($resultsCfg['status_code']['all'], $targetExecStatus)) ) 
		{
			 ...
			 * Author : QASource
			 * Defect ID : 5751
			 * With our new architecture change to load execution tree
			 * this function is not required.
			 * But this function is used with other parameters
			 * That's why we didn't delete this function.
			 **/
			 
			 
			if($_GET['type'] == 'testplan' || $_GET['type'] == 'testreport' ){
		...
		if (isset($my['filters']['filter_custom_fields']) && isset($test_spec['childNodes']))
        {
			 ...
			 * Author : QASource
			 * Defect ID : 5751
			 * With our new architecture to load execution tree
			 * this feature is not required.
			 * But this feature is used with other parameters
			 * That's why we didn't delete this feature.
			 **/
			if($_GET['type'] == 'testplan' || $_GET['type'] == 'testreport' ){
		...
		$menustring = renderExecTreeNode(1,$test_spec,$tplan_tcases,$hash_id_descr,1,$menuUrl,
			                             ...
			                             $showTestCaseID,$tcase_prefix,$show_testsuite_contents,$tplan_id);
	}  // if($test_spec)
                      ...
                      $tplan_name,$objFilters,$objOptions) 
{
	...
	if($test_spec)
	{
		...
		// echo "<pre>";print_r($test_spec['childNodes']);
		// Take time
	 	//$chronos[] = microtime(true);
		//$tnow = end($chronos);
		//$tprev = prev($chronos);
