...
if ($args->do_upload)
{
	...
	{ 
		...
		if (move_uploaded_file($source, $gui->dest))
		{
			...
			switch($args->importType)
			{
				...
				case 'XLS':
					...
					{
						//$pcheck_fn = "check_xml_tc_tsuite";
						$pimport_fn = "importTestSuiteDataFromSpreadsheet";
						break;
					}else{
						$pimport_fn = "importTestCaseDataFromSpreadsheet";
						break;
		...
		{
			/* echo $_FILES['uploadedFile']['type']; die;
			// application/octet-stream
			// application/vnd.oasis.opendocument.spreadsheet
			die; */
			if($pimport_fn == 'importTestSuiteDataFromSpreadsheet' || $pimport_fn == 'importTestCaseDataFromSpreadsheet'){
				if($_FILES['uploadedFile']['type'] == 'application/octet-stream'){
					...
					if(!empty($gui->resultMap) && !is_array($gui->resultMap)){
						$gui->file_check = array('status_ok' => 0, 'msg' => $gui->resultMap);
						$args->importType = null;
						$gui->resultMap	=	null;
					}
				}else{
					$gui->file_check = array('status_ok' => 0, 'msg' => 'Only XLS file format is supported');
					$args->importType = null;
					$gui->resultMap	=	null;
				}
			}else{
                            ...
                            $userID,$kwMap,$duplicatedLogic = array('hitCriteria' => 'name', 'actionOnHit' => null))
{
	...
	for($idx = 0; $idx <$tc_qty ; $idx++)
	{
		...
		if($name_len > $fieldSizeCfg->testcase_name)
		{
			...
			/*
				Bug id      :2628
				Description :system is still  displaying original title under field "Summary" if the value for column "title" contains than 255 characters in the uploaded XLS file
				Done by     : Amit Sharma
			*/
                                           ...
                                           $userID,$useRecursion,$importIntoProject = 0)
{
	...
	$check_data	=	create_xml_tcspec_from_xls($fileName,$xml_filename);
	if(!empty($check_data)){
		return $check_data;
	}
...
function create_xml_tcspec_from_xls($xls_filename,$xml_filename) 
{
	...
	if(empty($xls_rows[1][1])){
		...
		$flag	=	True;
	}elseif(empty($xls_rows[1][3])){
		$flag	=	True;
	}elseif(empty($xls_rows[1][4])){
		$flag	=	True;
	}elseif(empty($xls_rows[1][5])){
		$flag	=	True;
	}elseif(empty($xls_rows[1][6])){
		$flag	=	True;
	}elseif(empty($xls_rows[1][7])){
		$flag	=	True;
	}
	
	if($flag){
		return "Problem in Excel format.First row elements must be of predefined format";
	}
	
	if($xls_row_qty < FIRST_DATA_ROW)
	{
		
    	return "There is only one row in file.";  // >>>----> bye!
  	}
	for($idx = FIRST_DATA_ROW; $idx <= $xls_row_qty; $idx++ )
	{
		$t_component = htmlspecialchars($xls_rows[$idx][1]);
		$t_category = htmlspecialchars($xls_rows[$idx][2]);
		$t_name		= htmlspecialchars($xls_rows[$idx][3]);
		if(empty($t_component)){
			return "Component Field cannot be blank.";
			break;
		}elseif(empty($t_name)){
			return "Title of test case cannot be blank.";
			break;
		}elseif(empty($t_category)){
...
/*
   Done By : Amit Sharma
   Description :
				function: importTestSuiteDataFromSpreadsheet
				 convert a XLS file to XML, and call importTestSuiteDataFromXML() to do import.
...
*/
	...
	function importTestSuiteDataFromSpreadsheet(&$db,$fileName,$parentID,$tproject_id,
											   $userID,$useRecursion,$importIntoProject = 0)
	{
		...
		$check_data	=	create_xml_testsuitespec_from_xls($fileName,$xml_filename);
...
/*
   Done By : Amit Sharma
  Description :
                function: create_xml_testsuitespec_from_xls
            Using an XSL file, that contains testsuite specifications
            creates an XML testlink test specification file.
            ...
            First row contains header:  name,summary,steps,expectedresults,keywords
            and must be skipped.
...
*/
	...
	function create_xml_testsuitespec_from_xls($xls_filename,$xml_filename) 
	{
		define('FIRST_DATA_ROW',2);
		define('IDX_COL_TESSUITE',1);
		define('IDX_COL_TESTSUITENAME',2);
		define('IDX_COL_NAME',3);
		define('IDX_COL_SUMMARY',4);
		define('IDX_COL_ACTIONS',5);
		define('IDX_COL_EXPRESULTS',6);
		define('IDX_COL_KEYWORDS',7);
		//define('IDX_COL_PRECONDITIONS',8);
		$xls_handle = new Spreadsheet_Excel_Reader();
		$xls_handle->setOutputEncoding(config_get('charset')); 
		$xls_handle->read($xls_filename);
		$xls_rows = $xls_handle->sheets[0]['cells'];
		...
		for($idx = FIRST_DATA_ROW; $idx <= $xls_row_qty; $idx++ )
		{
			$testcomponent_name = htmlspecialchars($xls_rows[$idx][IDX_COL_TESSUITE]);
			fwrite($xmlFileHandle,"<testsuite name=" . '"' . $testcomponent_name. '"'.">\n");
			//$testcomponent_name1 = htmlspecialchars($xls_rows[$idx][IDX_COL_TESSUITE]);
			$testsuitename = htmlspecialchars($xls_rows[$idx][IDX_COL_TESTSUITENAME]);
			/*
			  Done By : Amit Sharma
			*/
			if($testsuitename!='')
			{
				fwrite($xmlFileHandle,"<testsuite name=" . '"' . $testsuitename. '"'.">\n");
			}
			else
			{
				echo "Please check your excel file format";
				exit();
			}
			/* code ended here */
			$name 	= htmlspecialchars($xls_rows[$idx][IDX_COL_NAME]);
			...
			*/
			 //$Preconditions = convert_special_char($xls_rows[$idx][IDX_COL_PRECONDITIONS]);
			 /* ended here */
			$keywords = convert_special_char($xls_rows[$idx][IDX_COL_KEYWORDS]);
			//$keywords = nl2p(htmlspecialchars($keywords));
			$step_number = 1;
			...
			*/
			...
			//else
			//{
				// fwrite($xmlFileHandle,"<preconditions><![CDATA[]]></preconditions>\n");
			...
			fwrite($xmlFileHandle,"<keywords>\n");
			fwrite($xmlFileHandle,"<keyword name=" . '"' . $keywords. '"'.">\n");
			fwrite($xmlFileHandle,"<notes> <![CDATA[]]> </notes>\n");
			fwrite($xmlFileHandle,"</keyword>\n");
			fwrite($xmlFileHandle,"</keywords>\n");
