...
{literal}
	<script type="text/javascript">
		function valid_type()
		{
			var filetypedata = document.getElementById('importType').value;
			var getlastextdata = filetypedata.toLowerCase();
...
<form method="post" enctype="multipart/form-data" action="{$SCRIPT_NAME}" onsubmit="return valid_type();">
  ...
  <td>
	...
	*}
	...
	<select name="importType" id="importType">
		<option value="XML" label="XML">XML</option>
         {*html_options options=$gui->importTypes*}
	</select>
	...
	<tr><td>{$labels.local_file} </td>
		...
		<td><input type="file" name="uploadedFile" id="uploadedFile"
	                           size="{#FILENAME_SIZE#}" maxlength="{#FILENAME_MAXLEN#}" onBlur="vali_type();"/></td>
	</tr>
	{if $gui->hitOptions != ''}
...
//BUGID 3943: Escape all messages (string)
 // alert_message("{$labels.warning|escape:'javascript'}","{$gui->file_check.msg|escape:'javascript'}");
	 // alert('Please Upload file in the selected File Type only');
